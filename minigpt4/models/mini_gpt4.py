...
from minigpt4.models.xlmr import xlmr
...
class MiniGPT4(Blip2Base):
    ...
    def __init__(
        ...
        q_former_model="/home/cchuan/Project/MiniGPT-4/weight/Qformer/blip2_pretrained_flant5xxl.pth",
        img_size=224,
        drop_path_rate=0,
        ...
        if self.has_qformer:
            ...
            # 这里改成xlmr的输出大小
            self.Qformer, self.query_tokens = self.init_Qformer(
                num_query_token, self.xlmr.num_features
            )
    ...
    def encode_img(self, image):
        ...
        with self.maybe_autocast():
            ...
            # 原先VIT对一张图像产生[1, 257, 1408]的特征，类型是torch
            image_embeds = self.xlmr.extract_features('hello world!').to(device)

            if self.has_qformer:
                #  image是一个全1的向量，形状是(batch, num_token)
                image_atts = torch.ones(image_embeds.size()[:-1], dtype=torch.long).to(device)
                # self.query_tokens是一个(1, num_query_token, hidden_size)
                query_tokens = self.query_tokens.expand(image_embeds.shape[0], -1, -1)
                # 这里query_tokens就变成了(batch_size, num_query_token, hidden_size)
    ...
    def forward(self, samples):
        # 这里输入一个samples
        # 这里面包含了image，instruction_input，answer
        image = samples["image"]
    ...
    def from_config(cls, cfg):
        ...
        q_former_model = cfg.get("q_former_model", "/home/cchuan/Project/MiniGPT-4/weight/Qformer/blip2_pretrained_flant5xxl.pth")

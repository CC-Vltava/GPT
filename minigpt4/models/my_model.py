...
class GPT(nn.Module):
    def __init__(
            self,
            num_query_token=32,
            llama_model="/data1/cchuan/data/weight/vicuna/",
    ...
    def proceed_in_qformer(self, text):
        device = 'cpu'
        text_atts = torch.ones(text.size()[:-1], dtype=torch.long).to(device)
        text = text.to(device)
        query_tokens = self.query_tokens.expand(text.shape[0], -1, -1)

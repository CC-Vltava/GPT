...
class GPT(nn.Module):
    ...
    def load_model_data(self, model_path=None):
...
def read_data():
    ...
    # sample里面含有prompt和completion两个部分，每个是一个list，装有对应的语句和answer
    return samples
...
my_model = GPT()
my_model.load_model_data('/data1/cchuan/model_weight/GPT_1M/5/pytorch_model.bin')
my_model.to_device(GPU_DEVICE)


output = my_model.generate(sample)
